...
export const checkPromo = async (condition, user, qty, cartValue, paymentMethod, shippingMethod, items) => {
    let fo = true
    let cv = true
    let cq = true
    let pm = true
...
export const calculateOffers = async (req) => {
    let paymentMethod = req.body.paymentMethod
    let items = req.body.items
    let qty: Number = req.body.qty
    let total = +req.body.total
    let details: any = null
    let promotions = await Promotion.find({ type: 'cart', active: true, validFromDate: { $lte: new Date() }, validToDate: { $gte: new Date() } }).select('name description condition action condition action validFromDate validToDate').sort('priority')
    let { shipping } = await Settings.findOne({}).select('shipping')
    let amount = total + shipping.charge
    for (let o of promotions) {
        let pro = await this.checkPromo(o.condition, req.user, qty, total, paymentMethod, shipping.method, items)
        let val = 0
        if (!pro) continue
        if (o.action.key == 'FreeShipping' && shipping.charge > 0) {
            ...
            val = (o.action.type == 'Percent') ?
                amount * +o.action.val / 100 : +o.action.val
            details = o
        }
