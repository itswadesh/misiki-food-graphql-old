...
export const sms = async ({ phone, msg, otp }: any) => {
  try {
    ...
    if (otp) {
      ...
      fetch(smsString).then(res => console.log).catch(e => console.error)
      console.log(`${phone} = ${otp}`)
    } else {
      fetch(`http://78.46.85.205:5684/api/SendSMS?api_id=API12517803163&api_password=12345678&sms_type=T&encoding=T&sender_id=MISIKI&phonenumber=${phone}&textmessage=${msg}`)
