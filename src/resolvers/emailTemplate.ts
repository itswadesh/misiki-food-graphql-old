...
import { Request, MessageDocument, UserDocument, EmailTemplateDocument } from '../types'
...
const resolvers: IResolvers = {
  Query: {
    emailTemplates: async (root, args, ctx, info) => {
      const result: any = await fs.readFileSync(`${STATIC_PATH}/templates/${args.folder}/${args.name}.hbs`, "utf8");
      if (!result) throw new UserInputError("Page not found");
      else return result
    },
    emailTemplate: async (root, args: { id: string, name: string }, ctx, info): Promise<EmailTemplateDocument | null> => {
      const result: any = await fs.readFileSync(`${STATIC_PATH}/templates/${args.name}.hbs`, "utf8");
      if (!result) throw new UserInputError("Page not found");
      else return result;
    }
  },
  Mutation: {
    ...
    // },
    saveEmailTemplate: async (root, args, { req }: { req: Request }): Promise<EmailTemplateDocument> => {
      return fs.writeFileSync(`${STATIC_PATH}/templates/${args.name}.hbs`, args.content, "utf8");
    }
  }
}
