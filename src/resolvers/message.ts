...
const resolvers: IResolvers = {
  Query: {
    ...
    message: async (root, args: { id: string }, ctx, info): Promise<MessageDocument | null> => {
      return Message.findById(args.id, fields(info))
    }
  },
  Mutation: {
    removeMessage: async (root, args, { req }: { req: Request }): Promise<MessageDocument | null> => {
    ...
    saveMessage: async (root, args, { req }: { req: Request }): Promise<MessageDocument | null> => {
      const { userId } = req.session
      const message = await Message.findOneAndUpdate({ _id: args.id || Types.ObjectId() }, { $set: { ...args, user: userId } }, { upsert: true, new: true })
